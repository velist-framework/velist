<script lang="ts">
  ...
  let otherUsers = $derived(users.filter(u => u.id !== user.id))
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
            ...
            <tr class="bg-indigo-50/50 dark:bg-indigo-900/10">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center text-white font-semibold ring-2 ring-indigo-200 dark:ring-indigo-800">
                    {user.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p class="font-medium text-gray-900 dark:text-white">{user.name} <span class="text-xs text-indigo-600 dark:text-indigo-400">(You)</span></p>
              ...
              <td class="px-6 py-4">
                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300">
